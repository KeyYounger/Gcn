...
class DiGCN(nn.Module):
    def __init__(self):
        ...
        self.s_att = Spatial_Attention(4, 8, 0.01)

    def forward(self, x): # x.shape = (batch, nums_node, in_feats, time)
        x_embedding = F.relu(self.lin(x.permute(0, 3, 1, 2)))
        out = self.s_att(x_embedding)
        return out


class Spatial_Attention(nn.Module):
    def __init__(self, n_heads, in_feats, dropout):
        '''
        :param n_heads:
        :param in_feats:
        :param dropout:
        ...
        self.W_q = nn.Parameter(torch.FloatTensor(in_feats, in_feats))
        self.W_k = nn.Parameter(torch.FloatTensor(in_feats, in_feats))
        self.W_v = nn.Parameter(torch.FloatTensor(in_feats, in_feats))
    ...
    def forward(self, x):
        '''
        :param x: (batch, time, nums_node, in_feats)
        :return: (batch, time, nums_node, in_feats)
        '''
        Q = torch.matmul(x, self.W_q)  # Q.shape = (b, t, n, i)
        K = torch.matmul(x, self.W_k)  # K.shape = (b, t, n, i)
        V = torch.matmul(x, self.W_v)  # V.shape = (b, t, n, i)

        split_size = int(Q.shape[-1] / self.n_heads)  # 每个head的维度
        # shape = (b, t, heads, n, i/split_size)
        q = Q.view(x.shape[0], x.shape[1], x.shape[2], self.n_heads, split_size).permute(0, 1, 3, 2, 4)
        k = K.view(x.shape[0], x.shape[1], x.shape[2], self.n_heads, split_size).permute(0, 1, 3, 2, 4)
        v = V.view(x.shape[0], x.shape[1], x.shape[2], self.n_heads, split_size).permute(0, 1, 3, 2, 4)
        # attention = (b, t, heads, n, n)
        attention = torch.matmul(q, k.permute(0, 1, 2, 4, 3)) / torch.sqrt(torch.FloatTensor([split_size]))
        attention = torch.softmax(attention, dim=-1)

        x = torch.matmul(attention, v).permute(0, 1, 3, 2, 4).contiguous()
        x = x.view(x.shape[0], x.shape[1], -1, self.n_heads * split_size)
        x = self.fc(x)
...
train_x = file_data['test_x']  # (10181, 307, 3, 12)
train_x = train_x[:, :, 0:1, :]
